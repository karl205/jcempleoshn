            ...
            <tbody>
                @foreach ($postulantes as $postulante)
                    <tr>
                        ...
                        <td id="acciones-{{ $postulante['id'] }}" class="d-flex gap-1">
                            <button class="btn btn-sm btn-outline-info" data-bs-toggle="modal"
                                data-bs-target="#perfilModal{{ $postulante['id'] }}">
                                <i class="bi bi-eye-fill"></i>
                            </button>
                            <div class="btn-group btn-group-sm" role="group">
                                <button type="button" class="btn btn-outline-success btn-apto"
                                    data-id="{{ $postulante['id'] }}">Apto</button>
                                <button type="button" class="btn btn-outline-danger btn-noapto"
                                    data-id="{{ $postulante['id'] }}">No Apto</button>
                            </div>
                        </td>

                        <td>
                            <div class="d-flex flex-column gap-1">
                                <textarea class="form-control form-control-sm" rows="2" placeholder="Escriba un comentario..."></textarea>
                    ...
                    @php
                        ...
                        // Plaza aplicada
                        $plazaCargo = $plazaCargo ?? (data_get($plaza ?? [], 'cargo') ?? '—');
                        ...
                        $email = data_get($postulante, 'email', '—');
                    ...
                    <div class="modal fade" id="perfilModal{{ $postulante['id'] }}" tabindex="-1"
                        aria-labelledby="perfilLabel{{ $postulante['id'] }}" aria-hidden="true">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <ul class="nav nav-tabs" role="tablist">
                                        <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab"
                                                data-bs-target="#pane-personal-{{ $postulante['id'] }}" type="button"
                                                role="tab">Personal</button></li>
                                        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab"
                                                data-bs-target="#pane-academica-{{ $postulante['id'] }}" type="button"
                                                role="tab">Académica</button></li>
                                        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab"
                                                data-bs-target="#pane-experiencia-{{ $postulante['id'] }}" type="button"
                                                role="tab">Experiencia</button></li>
                                        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab"
                                                data-bs-target="#pane-idiomas-{{ $postulante['id'] }}" type="button"
                                                role="tab">Idiomas</button></li>
                                    </ul>

                                    <div class="tab-content pt-3">
                                        <!-- PERSONAL -->
                                        <div class="tab-pane fade show active" id="pane-personal-{{ $id }}"
                                            role="tabpanel">
                                            <div class="row g-3 align-items-start">
                                                <div class="col-md-8">
                                                    <div class="row g-3">
                                                        <div class="col-md-6"><strong>Nombres:</strong>
                                                            {{ data_get($postulante, 'nombres', '—') }}</div>
                                                        <div class="col-md-6"><strong>Apellidos:</strong>
                                                            {{ data_get($postulante, 'apellidos', '—') }}</div>
                                                        ...
                                                        <div class="col-md-6"><strong>Sexo:</strong>
                                                            {{ ucfirst(data_get($postulante, 'sexo', '—')) }}</div>
                                                        <div class="col-md-6"><strong>Teléfono:</strong>
                                                            {{ data_get($postulante, 'telefono', '—') }}</div>
                                                        <div class="col-md-6"><strong>Correo:</strong>
                                                            {{ data_get($postulante, 'correo', '—') }}</div>
                                                        <div class="col-md-6"><strong>Vehículo:</strong>
                                                            {{ ucfirst(data_get($postulante, 'vehiculo', '—')) }}</div>
                                                        <div class="col-md-6"><strong>Departamento:</strong>
                                                            {{ data_get($postulante, 'departamento', '—') }}</div>
                                                        <div class="col-md-6"><strong>Ciudad:</strong>
                                                            {{ data_get($postulante, 'ciudad', '—') }}</div>
                                                        ...
                                                        <div class="col-md-6"><strong>Plaza aplicada:</strong>
                                                            {{ $plazaCargo }}</div>
                                                    </div>
                                                    <hr>
                                                    <p class="mb-1"><strong>Resumen profesional:</strong> Candidato con
                                                        experiencia en el área de
                                                        {{ $plazaCargo !== '—' ? strtolower($plazaCargo) : '—' }}.</p>
                                                </div>
                                        ...
                                        <!-- ACADÉMICA -->
                                        <div class="tab-pane fade" id="pane-academica-{{ $postulante['id'] }}"
                                            role="tabpanel">
                                                ...
                                                <div class="border rounded p-3 mb-3">
                                                    <div class="d-flex justify-content-between flex-wrap">
                                                        <div>
                                                            ...
                                                            <div class="text-muted small">{{ $edu['nivel'] ?? '—' }} ·
                                                                {{ $edu['area'] ?? '—' }} · {{ $edu['pais'] ?? '—' }}
                                                            </div>
                                                        </div>
                                        ...
                                        <!-- EXPERIENCIA -->
                                        <div class="tab-pane fade" id="pane-experiencia-{{ $postulante['id'] }}"
                                            role="tabpanel">
                                                ...
                                                <div class="border rounded p-3 mb-3">
                                                    <div class="d-flex justify-content-between flex-wrap">
                                                        <div>
                                                            ...
                                                            <div class="text-muted small">{{ $exp['patrono'] ?? '—' }} ·
                                                                {{ $exp['pais'] ?? '—' }}</div>
                                                            <div class="small">
                                                                ...
                                                                |
                                                                <strong>Cargo (según categoría):</strong>
                                                                {{ $exp['cargo_categoria'] ?? '—' }}
                                                            </div>
                                                        </div>
                                                        <div class="text-nowrap">
                                        ...
                                        <!-- IDIOMAS -->
                                        <div class="tab-pane fade" id="pane-idiomas-{{ $postulante['id'] }}"
                                            role="tabpanel">
                                                ...
                                                <ul class="list-group">
                                                    @foreach ($idiomas as $idi)
                                                        <li
                                                            ...
                                                            <span
                                                                class="badge bg-secondary">{{ $idi['nivel'] ?? '—' }}</span>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            @endif
                                ...
                                <div class="modal-footer">
                                    <small class="text-muted me-auto">
                                        Plaza: {{ $plaza['cargo'] ?? '—' }} ·
                                        Postulación:
                                        {{ isset($postulante['fecha']) ? \Carbon\Carbon::parse($postulante['fecha'])->format('d/m/Y') : '—' }}
                                    </small>

                                    <button type="button" class="btn btn-dark btn-print-cv"
